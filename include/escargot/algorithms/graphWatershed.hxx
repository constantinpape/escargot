...
namespace graphWatershed {
    ...
    template<class value_type, class label_type> label_type stream(coordinate_type const ,
            andres::Marray<value_type> const & , andres::Marray<value_type> const & ,
            andres::Marray<label_type> const & , std::vector< std::pair<size_t,size_t> > & );
    
    template<class value_type, class label_type> andres::Marray<value_type> nodeWeightsFromEdgeWeights(andres::Marray<value_type>  const &, value_type const, value_type const );

    // TODO replace this by a proper RAG
    template<class value_type, class label_type> std::map<coordinate_type, value_type> get_region_weights(andres::Marray<value_type> const & , andres::Marray<label_type> const & );
    
    template<class value_type, class label_type> andres::Marray<label_type> runGraphWatershed2d(andres::Marray<value_type> const &, andres::Marray<value_type> const & );
    
    template<class value_type, class label_type> void
    apply_size_filter_inplace(andres::Marray<label_type> &, std::map<coordinate_type, value_type> const &, size_t const, value_type const);


    // toplevel function for the watershed 
    // TODO check that shapes match
    template<class value_type, class label_type> andres::Marray<label_type> 
    graphWatershed2d(andres::Marray<value_type> edge_weights, value_type const upper_threshold, value_type const lower_threshold, value_type const size_threshold, value_type const region_threshold)
    {
        if( lower_threshold > upper_threshold )
            throw( std::runtime_error("Thresholds inverted!") );

        andres::Marray<value_type> node_weights = nodeWeightsFromEdgeWeights<value_type, label_type>(edge_weights, upper_threshold, lower_threshold);

        std::cout << "A" << std::endl;

        andres::Marray<label_type> labels = runGraphWatershed2d<value_type, label_type>(edge_weights, node_weights);
    ...
    template<class value_type, class label_type> andres::Marray<value_type> nodeWeightsFromEdgeWeights(andres::Marray<value_type>  const & edge_weights,
            value_type const upper_threshold, value_type const lower_threshold)
    {
        size_t shape[] = {edge_weights.shape(0), edge_weights.shape(1)};
        andres::Marray<value_type> ret(shape, shape + 2);
    ...
    andres::Marray<label_type> runGraphWatershed2d(andres::Marray<value_type> const & edge_weights, andres::Marray<value_type> const & node_weights)
    {
        size_t x_size = edge_weights.shape(0);
        size_t y_size = edge_weights.shape(1);
        
        size_t labels_shape[] = {x_size, y_size};
        andres::Marray<label_type> ret( labels_shape, labels_shape + 2 ); // I don't get this second shape, but it's everywhere in the marray examples
    ...
    get_region_weights(andres::Marray<value_type> const & edge_weights, andres::Marray<label_type> const & labels)
    {
    ...
    label_type stream(coordinate_type const pixel,
            andres::Marray<value_type> const & edge_weights, andres::Marray<value_type> const & node_weights,
            andres::Marray<label_type> const & labels, std::vector< std::pair<size_t,size_t> > & stream_coordinates )
    {
    ...
    template<class value_type, class label_type> void
    apply_size_filter_inplace(andres::Marray<label_type> & labels, std::map<coordinate_type, value_type> const & region_weights, size_t const size_threshold, value_type const region_threshold)
    {
        ...
        std::sort( region_weights_vec.begin(), region_weights_vec.end(), &sortByValDecreasing<value_type> );
        ...
        nifty::ufd::Ufd<label_type> ufd(n_regions);
